module NSWTopo
  def config(layer = nil, **options)
    delete, chrome, path, resolution, layer_dir, labelling, debug, gpu, zlib_level, knockout = options.values_at *%i[delete chrome path resolution layer-dir labelling debug gpu zlib-level knockout]
    ...
    Config.store("gpu", gpu) unless gpu.nil?
