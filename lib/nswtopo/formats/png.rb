module NSWTopo::Formats
  module Png
    def render_png(path, ppi:, dither: false, worldfile: false)
      if uptodate? "map.png", "map.svg"
        path.write read("map.png")
        return
        # TODO: maybe we should just pro-actively delete map.png whenever map.svg is updated
      end
      Dir.mktmppath do |temp_dir|
        svg_path = temp_dir / "map.svg"
        render_svg svg_path
        raise "TODO: rasterise svg_path to path here, also saving to archive as map.png"
      end
    end
  end
end

# def render_png(config, svg_path, png_path, temp_dir, ppi:, dither: nil)
