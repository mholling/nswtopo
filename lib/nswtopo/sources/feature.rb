module NSWTopo
  class FeatureSource
    ...
    def arcgis_features(source, options)
      ...
      oid_field = fields.values.find { |field| field["type"] == "esriFieldTypeOID" }
      oid_field_name, oid_field_alias = oid_field.values_at("name", "alias") if oid_field
      ...
      pages = Enumerator.new do |yielder|
        if options["definition"] && !service["supportsDynamicLayers"]
          ...
          index_attribute = options["page-by"] || source["page-by"] || oid_field_alias || oid_field_name || "OBJECTID"
          scale = options["scale"]
