  ...
  module Formats
    module KMZ
      ...
      def render_kmz(temp_dir, kmz_path, name:, ppi:, **options)
        ...
        degree_resolution = 180.0 * metre_resolution / Math::PI / EARTH_RADIUS

        wgs84_bounds = bounds(projection: Projection.wgs84)
        wgs84_dimensions = wgs84_bounds.transpose.difference / degree_resolution
        max_zoom = Math::log2(wgs84_dimensions.max).ceil - Math::log2(TILE_SIZE).to_i
        ...
        pyramid = (0..max_zoom).map do |zoom|
          ...
          dimensions = counts.times TILE_SIZE
          ...
          print "\r\033[Kresizing image pyramid: %i%%" % (100 * (2**(zoom + 1) - 1) / (2**(max_zoom + 1) - 1))
        ...
        Dir.mktmppath do |kmz_dir|
          pyramid.map do |zoom, (indices_bounds, tif_path)|
            ...
            indices_bounds.map do |indices, tile_bounds|
              ...
              crop = "%ix%i+%i+%s" % [ TILE_SIZE, TILE_SIZE, indices[0] * TILE_SIZE, indices[1] * TILE_SIZE ]
          ...
          xml.add_element("kml", "xmlns" => "http://earth.google.com/kml/2.1").tap do |kml|
            kml.add_element("Document").tap do |document|
              document.add_element("LookAt").tap do |look_at|
                range_x = @extents.first / 2.0 / Math::tan(FOV) / Math::cos(TILT)
                range_y = @extents.last / Math::cos(FOV - TILT) / 2 / (Math::tan(FOV - TILT) + Math::sin(TILT))
                names_values = [ %w[longitude latitude], GeoJSON.point(@centre, @projection).reproject_to_wgs84.coordinates ].transpose
                names_values << [ "tilt", TILT * 180.0 / Math::PI ] << [ "range", 1.2 * [ range_x, range_y ].max ] << [ "heading", -@rotation ]
