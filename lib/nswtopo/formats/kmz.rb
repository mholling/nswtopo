module NSWTopo
  module KMZ
    ...
    def self.build(ppi, image_path, kmz_path)
      wgs84_bounds = CONFIG.map.wgs84_bounds
      degrees_per_pixel = 180.0 * CONFIG.map.resolution_at(ppi) / Math::PI / EARTH_RADIUS
      ...
      Dir.mktmppath do |temp_dir|
        ...
        CONFIG.map.write_world_file worldfile_path, CONFIG.map.resolution_at(ppi)
        ...
        pyramid = (0..max_zoom).map do |zoom|
          ...
          %x[gdalwarp -s_srs "#{CONFIG.map.projection}" -t_srs "#{Projection.wgs84}" -r bilinear -dstalpha "#{source_path}" "#{tif_path}"]
        ...
        kmz_dir = temp_dir + CONFIG.map.filename
        kmz_dir.mkdir
        ...
        xml.add_element("kml", "xmlns" => "http://earth.google.com/kml/2.1").tap do |kml|
          kml.add_element("Document").tap do |document|
            document.add_element("LookAt").tap do |look_at|
              range_x = CONFIG.map.extents.first / 2.0 / Math::tan(FOV) / Math::cos(TILT)
              range_y = CONFIG.map.extents.last / Math::cos(FOV - TILT) / 2 / (Math::tan(FOV - TILT) + Math::sin(TILT))
              names_values = [ %w[longitude latitude], CONFIG.map.projection.reproject_to_wgs84(CONFIG.map.centre) ].transpose
              names_values << [ "tilt", TILT * 180.0 / Math::PI ] << [ "range", 1.2 * [ range_x, range_y ].max ] << [ "heading", -CONFIG.map.rotation ]
              names_values.each { |name, value| look_at.add_element(name).text = value }
            end
            document.add_element("Name").text = CONFIG.map.name
            document.add_element("Style").tap(&style)
        ...
        temp_kmz_path = temp_dir + "#{CONFIG.map.filename}.kmz"
