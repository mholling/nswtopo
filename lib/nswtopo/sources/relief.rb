module NSWTopo
  class ReliefSource
    ...
    def get_raster(temp_dir)
      ...
      bounds = CONFIG.map.bounds.map do |lower, upper|
      ...
      when params["contours"]
        ...
        %w[contours coastline].inject(nil) do |append, layer|
          ...
          when /^(https?:\/\/.*)\/\d+\/query$/
            ...
            geometry = CONFIG.map.projection.transform_bounds_to(service_projection, bounds).flatten.values_at(0,2,1,3).join(?,)
      ...
      %x[gdalwarp -s_srs "#{CONFIG.map.projection}" -t_srs "#{CONFIG.map.projection}" -srcnodata 0 -r bilinear -dstalpha "#{relief_path}" "#{tif_path}"]
