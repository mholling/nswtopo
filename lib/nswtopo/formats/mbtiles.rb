module NSWTopo
  module MBTiles
    ...
    def self.build(ppi, svg_path, temp_dir, mbt_path)
      sql = %Q[
        CREATE TABLE metadata (name TEXT, value TEXT);
        INSERT INTO metadata VALUES ("name", "#{CONFIG.map.name}");
        INSERT INTO metadata VALUES ("type", "baselayer");
        INSERT INTO metadata VALUES ("version", "1.1");
        INSERT INTO metadata VALUES ("description", "#{CONFIG.map.name}");
        INSERT INTO metadata VALUES ("format", "png");
        INSERT INTO metadata VALUES ("bounds", "#{CONFIG.map.wgs84_bounds.flatten.values_at(0,2,1,3).join ?,}");
        CREATE TABLE tiles (zoom_level INTEGER, tile_column INTEGER, tile_row INTEGER, tile_data BLOB);
      ]
      cosine = Math::cos(CONFIG.map.wgs84_bounds.last.mean * Math::PI / 180)
      bounds = CONFIG.map.transform_bounds_to(Projection.new "EPSG:3857")
      png_path = temp_dir + "#{CONFIG.map.filename}.mbtiles.png"
      (ppi <= 25 ? ppi : Math::log2(RESOLUTION * ppi * cosine / METERS_PER_INCH / CONFIG.map.scale).ceil).downto(0).inject([]) do |levels, zoom|
        ...
        tile_path = temp_dir.join("#{CONFIG.map.filename}.mbtiles.#{zoom}.%09d.png").to_s
        ...
        ppi = (METERS_PER_INCH * CONFIG.map.scale / resolution / cosine).ceil
        ...
        tif_path, tfw_path = %w[tif tfw].map { |ext| temp_dir + "#{CONFIG.map.filename}.mbtiles.#{zoom}.#{ext}" }
        ...
        %x[gdalwarp -s_srs "#{CONFIG.map.projection}" -t_srs EPSG:3857 -r lanczos -dstalpha "#{png_path}" "#{tif_path}"]
