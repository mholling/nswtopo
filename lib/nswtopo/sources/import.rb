module NSWTopo
  class ImportSource
    ...
    def resolution
      @resolution ||= begin
        ...
        Math::sqrt(0.5) * [ [ 0, 0 ], [ 1, 1 ] ].map do |point|
          %x[echo #{point.join ?\s} | gdaltransform "#{import_path}" -t_srs "#{CONFIG.map.projection}"].tap do |output|
    ...
    def get_raster(temp_dir)
      ...
      raise BadLayerError.new("couldn't use georeferenced file at #{import_path}") unless $?.success?
      %x[gdalwarp -t_srs "#{CONFIG.map.projection}" -r bilinear "#{source_path}" "#{tif_path}"]
